...
- [ ] 4.4 Confirm tests build and pass under the `USE_DEVICESETTINGS` build flag
    > **Blocked:** Requires `entservices-testframework` repo and full build environment. Run via GitHub Actions (`./bin/act -W .github/workflows/tests-trigger.yml`) or CI pipeline.

## 5. Final Verification

- [ ] 5.1 Run existing L1 test suite and confirm no regressions
    > **Blocked:** Same build environment prerequisite as 4.4.
- [x] 5.2 Re-run openspec coverage check and confirm gap G-01 is closed and overall score improves
    > `openspec/specs/spec_coverage.md` updated: G-01 marked resolved, score 81.5 → **83.5 / 100**, Code Coverage 36.5 → 38.5 / 40.
