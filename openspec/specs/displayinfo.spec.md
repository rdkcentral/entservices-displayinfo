...
#### Event payload — `isFrameRateChanged` flag (DeviceSettings backend only)

`Exchange::IConnectionProperties::INotification::Updated` carries a second parameter, `bool isFrameRateChanged`, alongside `Source`. On the DeviceSettings backend, `isFrameRateChanged` is `true` when a `POST_RESOLUTION_CHANGE` event is accompanied by a change in the connected display's frame rate (compared against a value cached at construction and after each prior `POST_RESOLUTION_CHANGE`); it is `false` for `POST_RESOLUTION_CHANGE` events with no frame-rate change, and always `false` for `PRE_RESOLUTION_CHANGE`/`HDMI_CHANGE`/`HDCP_CHANGE` on all backends. No new `Source` value is introduced.
...
- 2026-09-03 — displayinfo-framerate-change — Extended `Exchange::IConnectionProperties::INotification::Updated` with a second `isFrameRateChanged` boolean parameter (no new `Source` value); DeviceSettings backend caches the frame rate on construction and reports `isFrameRateChanged = true` on `POST_RESOLUTION_CHANGE` when the frame rate differs from the cached value.
